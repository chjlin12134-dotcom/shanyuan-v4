...
# 反方向的錯：為了怕被誤會逐字引用，反而講成「跟大師完全無關、是我自己原創」，
# 等於把大師教導的功勞佔為己有，跟編造出處是同一個問題的兩個方向。
DENIAL_OF_LINEAGE_PATTERN = re.compile(
    r"(不是來自|並非來自|跟大師|與大師).{0,12}(無關|沒有關係|某本書|的某本書)"
    r"|純粹是我自己的想法|完全是我自己原創|跟大師的教導無關"
)
...
def validate_buddhist_reply(text: str, verified_sources: list[str]) -> str | None:
    ...
    # 反方向的錯：講成「跟大師完全無關、是我自己原創」，等於把大師教導的
    # 功勞佔為己有，跟編造出處是同一個問題的兩個方向，一樣不誠實。
    if DENIAL_OF_LINEAGE_PATTERN.search(text):
        return "刻意撇清跟大師的淵源，把大師教導的內容講成完全是自己原創"
    return None
...
def format_retrieved(items: list[dict], buddhist_mode: bool = False) -> str:
    ...
    blocks.append(
        ...
        "第二種情況一律用第一人稱、為自己的話負責，標準說法是「我從大師的智慧得到啟發，我的理解是……」，"
        "也可以視語氣換成「我從大師這裡耳濡目染學到的是……」「我自己的體會是……」，"
        "不要包裝成大師說的話，也不要用「大師教導我們」這種聽起來像在轉述他人立場的講法。"
        "【很重要，不要犯反方向的錯】不逐字引用，不代表要撇清跟大師的關係——"
        "絕對不要說「這跟大師的教導無關」「並不是來自大師的某本書」「純粹是我自己的想法」這種刻意否認淵源的講法。"
        "你的體會本來就是從長期親近、熏習大師的教導而來，這個「受大師啟發」的淵源本身要老實講出來，"
        "不能因為怕被誤會成逐字引用，就反過來把大師教導的東西講成完全是你自己原創的想法——"
        "那樣等於把大師教導的功勞佔為己有，跟編造出處是同一個問題的兩個方向，一樣不誠實。"
        "拿不準到底是 (1) 還是 (2) 的時候，一律當作 (2) 處理：先誠實講成「我從大師的智慧得到啟發，我的理解是……」，"
    ...
    if buddhist_mode:
        ...
        if verified:
            ...
            source_check_instruction = (
                "\n\n---\n"
                f"【本輪提示：出處查證】{asked_directly}系統剛用你上一輪實際講的內容重新比對了語料庫，"
                "沒有查到夠相關、可以精準引用的出處。接下來如果要提到這件事，只能誠實告訴使用者：這不是逐字引用某一本書。"
                ...
                "同時也不要犯反方向的錯——不要說「這跟大師的教導無關」「純粹是我自己的想法」這種撇清淵源的話，"
                "要老實承認這是你受大師啟發、長期熏習之後融匯在自己身上的體會。因為這是承接你上一輪已經講過的話，"
                "標準說法是「我的這個理解是得自大師的啟發」，這種既不逐字引用、也不否認淵源的講法，不要把大師教導的功勞講成完全是你自己原創的。"
            ...
            if farewell:
                ...
                if violation:
                    ...
                    corrected_system = full_system + (
                        ...
                        "除非上面的『出處查證』真的給你一筆查到的出處，否則一律改用「我從大師的智慧得到啟發，我的理解是……」"
                        "（如果是承接你上一輪已經講過的話，用「我的這個理解是得自大師的啟發」）這種為自己的話負責的第一人稱說法，"
                        "但也不要反過來說「這跟大師無關」「純粹是我自己原創的想法」——要老實承認這是受大師啟發、"
                        "長期熏習後融匯在自己身上的體會，不要把大師教導的功勞講成完全是你自己原創的。\n"
                    )
