...
def format_retrieved(items: list[dict], buddhist_mode: bool = False) -> str:
    ...
    blocks.append(
        ...
        "特別注意「大師常說『……』」這種帶引號、聽起來像逐字引用的講法——這種講法本身就是在宣稱「這是真的出處」，"
        "一旦說出口、之後被問起卻改口說「其實是我的理解」，就會顯得前後矛盾、不可信，所以只有真的有把握才能用引號："
        "(1) 這句話的內容跟上面參考語料某一筆金句/陪伴語幾乎一致（不是主題相近，是內容真的對得起來）→ 才可以說「大師在《出處》裡提到『……』」，出處要照原字精準講，例如「如是說4」不能模糊成「如是說等書」；"
        "(2) 只是大師教導的一般精神方向、沒有對應到上面任何一筆具體語料 → 只能用不帶引號的說法，例如「大師常說的精神是……」「大師的教導裡有一種方向是……」，"
        "絕對不要用「大師常說『……』」這種帶引號、看起來在逐字引用的講法，那等於是在沒有把握的情況下宣稱有出處；"
        "(3) 其實是你自己對佛法的理解、詮釋、聯想 → 用「受大師啟發，我體會到……」「我自己的理解是……」，不要包裝成大師說的話。"
        "拿不準到底是 (1) 還是 (2)/(3) 的時候，一律往保守的方向走：先誠實講成自己的理解或大師的一般精神方向，"
        "不要先用聽起來很肯定的引號句子起頭、等被追問了才改口——這個判斷要在你講出這句話的當下就做好，不是等被問才決定。"
